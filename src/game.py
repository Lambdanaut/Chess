...
class Game:
    def __init__(self, aiOpponents=0, firstPlayer=pieces.w):
        ...
        if aiOpponents == 1:
            self.ai[pieces.ai] = bot.DepthBot(pieces.ai)
    ...
    def playerTurn(self, fails=0):
        ...
        if pieceOwner(piece) != self.player:
            if not pieceOwner(piece):
                printMe("Error: There is no piece in that position. Try again.")
            else:
                printMe("Error: You don't own that piece! Try again. ")
        ...
        if checkmate:
            ...
            printMe(display.showBoard(self.board))
            printMe(
                "Stale-mate! Player " + self.player + "'s King is safe where it is, but can't move anywhere without being in check. The game is a draw! ")
            exit()
